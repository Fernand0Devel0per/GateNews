...
namespace GateNewsApi.DAL.Interfaces
{
    public interface INewsDao : IBaseDao<News>
    {
        ...
        Task<(List<News> Items, int TotalPages)> GetByCategoryAsync(int categoryId, int pageNumber);
        Task<(List<News> Items, int TotalPages)> GetByCategoryAndAuthorAsync(int categoryId, string authorFullName, int pageNumber);
